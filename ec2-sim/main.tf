...
locals {
  ...
  region                       = "${var.region}"
  app_name                     = "${data.terraform_remote_state.common.iaps_app_name}"
...
module "create-ec2-instance" {
  ...
  app_name                    = "${local.environment_identifier}-${local.app_name}-${local.iaps_role}-001"
  ami_id                      = "${data.aws_ami.amazon_ami.id}"
...
resource "aws_route53_record" "instance" {
  ...
  name    = "${local.iaps_role}-001.${local.internal_domain}"
  type    = "A"
  ttl     = "300"
